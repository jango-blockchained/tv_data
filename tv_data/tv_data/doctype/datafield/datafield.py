...
#     formatted_date: str = adjusted_date.strftime("%Y%m%dT")
...
#     Returns:
#         str: The generated name in the format "DATA_{hash_code}_{key.upper()}".
#     """
#     hash_code: str = frappe.generate_hash(length=16).upper()
#     return f"DATA_{hash_code}_{key.upper()}"
...
#             self.name = generate_name(self.key)
...
class Datafield(Document):
    ...
    def set_scale(self) -> None:
        ...
        try:
            ...
            self.pricescale = 10 ** len(fractional_part)
        except ValueError:
            self.pricescale = 1
